...
internal static class TestyTraderTasks
{
    ...
    internal static unsafe bool ConfirmTrade()
    {
        if (TryGetAddonByName<AtkUnitBase>("Trade", out var addon) && IsAddonReady(addon))
        {
            ...
            if (tradeButton->Click(addon))
            {
                TaskLog.Verbose("Click Trade");
                return true;
            }
        }
    ...
    internal static async Task Trade(Dictionary<uint, uint> tradeDict, bool includeArmory, CancellationToken token = default)
    {
        ...
        {
            var inventoryGil = (uint)InventoryHelper.GetInventoryItemCount(1, checkArmory: includeArmory);
            var neededGil    = tradeDict[1];

            gilToTrade = Math.Min(neededGil, Math.Min(inventoryGil, 1_000_000u));

            tradeDict[1] -= gilToTrade;
            if (tradeDict[1] == 0 || gilToTrade == inventoryGil) tradeDict.Remove(1);
        }
        ...
        unsafe
        {
            ...
            var        itemAmount      = 0;
            TaskLog.Debug($"Total Itemtypes to trade: {tradeDict.Count}");
            var inventories = includeArmory
                                      ? InventoryHelper.MainInventory.Concat(InventoryHelper.Armory)
                                      : InventoryHelper.MainInventory;
            var inventoryTypes = inventories as InventoryType[] ?? inventories.ToArray();

            foreach (var item in tradeDict)
            {
                TaskLog.Debug($"Item: {item.Key} | Amount: {tradeDict[item.Key]}");
                if (item.Key == 1)
                    continue;
                if (itemAmount == 5)
                    break;
                ...
                foreach (var type in inventoryTypes)
                {
                    ...
                    for (var i = 0; i < cont->Size; i++)
                    {
                        ...
                        {
                            if (slot.IsHighQuality() != isHq)
                                continue;
                            var quantity = tradeDict[item.Key] < slot.Quantity
                                                   ? tradeDict[item.Key]
                                                   : (uint)slot.Quantity;
                            TaskLog.Debug($"Previous trade amount: {tradeDict[item.Key]} | Quantity to pass: {quantity}");
                            tradeDict[item.Key] -= quantity;
                            TaskLog.Debug($"Open trade amount: {tradeDict[item.Key]}");
        ...
        TaskLog.Debug($"Current tradeQueue amount: {tradeQueue.Count}");
        TaskLog.Debug($"Gil to trade: {gilToTrade}");
    ...
    private static unsafe bool TryAddItemHandin(QueueEntry entry, int slot)
    {
        if (TryGetAddonByName<AtkUnitBase>("Trade", out var addon) && IsAddonReady(addon))
        {
            if (InventoryHelper.GetItemSlot(InventoryType.HandIn, slot)
                               .ItemId !=
                0) return true;
            AgentTrade.Instance()->HandleCallback((ushort)entry.SlotID, entry.Type, default, 0);
        }

        return false;
    }

    internal static unsafe Dictionary<uint, int> GetCurrentInventory(bool includeArmory)
    {
