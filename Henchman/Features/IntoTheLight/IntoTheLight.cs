...
public class IntoTheLight : Module
{
    private static readonly int[] PublicDataCenters = [1, 2, 3, 10, 4, 5, 8, 11, 6, 7, 12, 9];
    private                 int   index;

    private static Configuration? Configuration => GetFeatureConfig<IntoTheLightUI, Configuration>();

    public override void RunTask() => TryStartTask(new TaskRecord(Start, "Into The Light"));
        ...
        for (index = 0; index < Configuration!.LightCharacters.Count; index++)
        {
            ...
            unsafe
            {
                if (AgentModule.Instance()->GetAgentLobby()->DataCenter != Configuration!.LightCharacters[index].DataCenterId)
                {
                    ...
                    {
                        charaSelectReturn->FireCallback(true, 19);
                ...
                var takenName = WaitUntilAsync(() =>
                                               {
                                                   unsafe
                                                   {
                                                       ...
                                                       {
                                                           addon->GetComponentButtonById(4)->Click(&addon->AtkUnitBase);
                                                           return true;
                                                       }
            ...
            {
                TaskLog.Debug("Progressing without queue!");
                await WaitUntilAsync(() => Svc.Condition[ConditionFlag.OccupiedInCutSceneEvent], "Wait for cutscene", token);

                await WaitUntilAsync(() => SkipCutscene(token), "Skip cutscene", token);
    ...
    private unsafe bool SelectDataCenterMenu()
    {
        ...
        {
            Log.Information("Visible");
            return true;
        }

        if (TryGetAddonByName<AtkUnitBase>("_TitleMenu", out var titleMenuAddon) && IsAddonReady(titleMenuAddon))
            titleMenuAddon->GetComponentButtonById(5)->Click(titleMenuAddon);

        return false;
    }

    private unsafe bool SelectDataCenter(int dc)
    {
        if (!PublicDataCenters.Contains(dc))
            return false;

        var data = Svc.Data.GetExcelSheet<WorldDCGroupType>()
                     ?.GetRowOrDefault((uint)dc);
        if (data == null ||
            data.Value.Name.ExtractText()
                .IsNullOrEmpty() ||
            data.Value.Region.RowId == 0)
            return false;

        if (TryGetAddonByName<AtkUnitBase>("TitleDCWorldMap", out var dcWorldMapAddon) && IsAddonReady(dcWorldMapAddon))
        {
            dcWorldMapAddon->FireCallback(true, 17, dc);
            return true;
        }
    ...
    private unsafe bool SelectCreateCharacter()
    {
        ...
        {
            charaSelectListMenuAddon->FireCallback(true, 29, 0, -1);
            return true;
        }
    ...
    private unsafe bool SelectIfUsePreset(bool usePreset)
    {
        ...
        {
            selectYesNoAddon->FireCallback(true, usePreset
                                                         ? 6
                                                         : 7);
            return true;
        }
    ...
    private bool RandomizeCharacterLook()
    {
        unsafe
        {
            ...
            {
                charaMakeFeatureAddon->FireCallback(true, -9, 0);
                return true;
            }
        }
    ...
    private unsafe bool ChooseClass(uint classJobId)
    {
        if (TryGetAddonByName<AtkUnitBase>("_CharaMakeProgress", out var charaMakeProgressAddon) && IsAddonReady(charaMakeProgressAddon))
        {
            charaMakeProgressAddon->FireCallback(true, 5, classJobId, -1, 0, string.Empty, 0);
