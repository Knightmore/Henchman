...
[Module]
public class IntoTheLightUI : ModuleUI<IntoTheLight, Configuration>
{
        ...
        DrawCentered("##LightStart", () =>
                                     {
                                         Layout.DrawButton(() =>
                                                           {
                                                               if (ImGui.Button(T("Start"), new Vector2(70 * GlobalFontScale, 30 * GlobalFontScale)) && !IsTaskRunning(Name)) Feature.RunTask();
                                                           });
                                     });
                                              ...
                                              drawExtraRow: () =>
                                                            {
                                                                ...
                                                                row.TableNextColumn();
                                                                ImGui.SetNextItemWidth(110f * GlobalFontScale);
                                                                if (ExcelSheetCombo<World>("##newworld", out var selectedWorld, s => s.GetRowOrDefault(newCharacter.WorldId) is { } row
                                                                                                                                             ? row.Name.ExtractText()
                                                                ...
                                                                row.TableNextColumn();
                                                                ImGui.SetNextItemWidth(100f * GlobalFontScale);
                                                                EnumCombo("##newclassJob", ref newCharacter.ClassJob);
                                                                ...
                                                                if (Combo("##newpreset", ref denseSelected, denseIds, names: names))
                                                                {
