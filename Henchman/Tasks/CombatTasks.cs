...
{
    internal static bool HandlingHaters { get; private set; }
    ...
    {
        TaskLog.Debug("Process Hunt Marks");
        foreach (var mark in huntMarks)
        {
            ...
            TaskLog.Debug($"Needed Kills: {(huntLog ? mark.GetOpenMonsterNoteKills : mark.GetOpenMobHuntKills)}");
            ...
            while (retries < 3)
            {
                ...
                if (mark.FateId == 0)
                {
                    TaskLog.Verbose($"{mark.MobHuntRowId} | {mark.MobHuntSubRowId} | {mark.GetMobHuntOrderRow.MobHuntReward.RowId}");
                    if (huntLog || (!huntLog && mark.GetMobHuntOrderRow.MobHuntReward.RowId % 4 != 3))
                    {
                        if (mark.Positions.Count > 0)
                        {
                            ...
                            for (var i = 0;;)
                            {
                                ...
                                TaskLog.Debug($"Trying position {i + 1}/{distanceOrderedPositions.Count} for {mark.Name}");

                                // TODO: Remove once MappingTheRealm is done. This is a reroute to properly get inside the underwater dome.
                                if (Player.TerritoryId == 613)
                                {
                                ...
                                else if (Player.TerritoryId == 138 && !Player.CanFly)
                                {
                                    if (markPosition.ToVector2()
                                ...
                                else if (Player.TerritoryId == 137 && !Player.CanFly)
                                {
                                ...
                                if (Vnavmesh.QueryMeshPointOnFloor.Invoke(markPosition with { Y = markPosition.Y + 5 }, false, 0.05f) is { } landablePosition)
                                    await MoveToArea(landablePosition, token);
                                else
                                    await MoveToArea(markPosition, token);

                                await WaitForPathToSettle("Wait for moving to Area", token);
                ...
                else
                {
                    ...
                    if (Vnavmesh.QueryMeshPointOnFloor.Invoke(fatePosition with { Y = fatePosition.Y + 20 }, false, 0.05f) is { } landableFatePosition)
                        await MoveToArea(landableFatePosition, token);
                    else
                        await MoveToArea(fatePosition, token);

                    await WaitForPathToSettle("Wait for walking to Area", token);
    ...
    {
        TaskLog.Debug("Process Duty Marks");
        token.ThrowIfCancellationRequested();
        ...
        foreach (var duty in duties)
        {
            ...
            {
                if (!SubscriptionManager.IsLoaded(IPCNames.Questionable)) FullError($"Questionable not enabled, can't do unlock quest! Skipping duty {duty}!");
                else
                {
    ...
    private static async Task<KillResult> KillCountedHuntMark(HuntMark huntMark, bool huntLog, int currentRank, bool gcLog, CancellationToken token = default)
    {
                       ...
                       : huntMark.GetOpenMobHuntKills     > 0)
        {
            ...
            await WaitUntilAsync(() => openKills !=
                                       (huntLog
                                                ? huntMark.GetOpenMonsterNoteKills
                                                : huntMark.GetOpenMobHuntKills), "Wait for kill to register", token);
            ...
            CancellationToken token   = default)
    {
        ...
        if (isCountedHuntMark && huntMark != null)
        {
            ...
            Log.Information("Wait for registered kill or unconscious");
            while (openKills ==
                   (isHuntLog
                            ? huntMark.GetOpenMonsterNoteKills
                            : huntMark.GetOpenMobHuntKills) &&
            ...
            {
                if (Svc.Targets.Target == null && !mob.IsDead) Svc.Targets.Target = mob;
                if (!mob.IsValid() || mob.IsDead) break;
    ...
    internal static async Task<bool> HandleHaters(uint nameId = 0, CancellationToken token = default)
    {
        ...
        try
        {
            return await HandleHatersInternal(nameId, token);
        } finally
        {
            HandlingHaters = false;
        }
    }
    ...
    private static async Task<bool> HandleHatersInternal(uint nameId, CancellationToken token)
    {
        ...
        while (GetHaterCount() > 0 &&
               Svc.Objects.Any(x => GetHaters()
                                   .ToArray()
                                   .Any(h => h.EntityId == x.EntityId && h.Enmity == 100 && Player.DistanceTo(x) <= 15f)))
        {
            var haters = GetHaters()
                   .ToArray();

            var haterArray = Svc.Objects.Where(x =>
                                                       haters.Any(h => h.EntityId == x.EntityId));


            Log.Information(string.Join("\n", haterArray.Select(o => $"[{o.EntityId}] {o.Name} @ {o.Position}")));

            var hater = Svc.Objects.FirstOrDefault(x =>
                                                           haters.Any(h => h.EntityId == x.EntityId) &&
                                                           !x.IsDead                                 &&
