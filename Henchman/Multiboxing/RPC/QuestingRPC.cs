        ...
        switch (dutyId)
        {
            ...
            case 1330 when !IsDutyUnlocked(1330): // Dzemael
                if (IsQuestAccepted(66515))
                    AbandonQuest(66515);
                await Questionable.CompleteQuest(Player.GrandCompany == (byte)GrandCompany.Maelstrom ? 66664u : Player.GrandCompany == (byte)GrandCompany.TwinAdder ? 66665u : 66666u, token);
                break;
            case 1331 when !IsDutyUnlocked(1331): // Aurum Vale
                if (IsQuestAccepted(66550))
                    AbandonQuest(66550);
                await Questionable.CompleteQuest(Player.GrandCompany == (byte)GrandCompany.Maelstrom ? 66667u : Player.GrandCompany == (byte)GrandCompany.TwinAdder ? 66668u : 66669u, token);
                break;
        }
