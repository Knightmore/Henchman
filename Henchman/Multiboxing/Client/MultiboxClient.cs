        ...
        CancellationToken                                                     token      = default)
{
    ...
    public async Task StartAsync()
    {
        ...
        TaskLog.Info("Connected to server.");
                        ...
                        if (Enum.TryParse<ServerRequest>(generalCommand, out var result))
                        {
                            switch (result)
                            {
                                case ServerRequest.ServerFull:
                                    TaskLog.Debug("Server full. Disconnecting client.");
                                    return;
                                case ServerRequest.Disconnect:
                                    TaskLog.Debug("Server requested disconnect.");
                                    return;
                                case ServerRequest.Turn:
                                {
                                    ...
                                    if (characters.TryGetFirst(out var nextCharacter))
                                    {
                                        ...
                                        if (Lifestream.IsBusy.Invoke())
                                        {
                                            await WaitUntilAsync(() => !Lifestream.IsBusy.Invoke(),
                                                                 "Waiting until Lifestream is done",
                                                                 token);
                                        }

                                        await Lifestream.SwitchToChar(nextCharacter.Name, nextCharacter.World, Lang.SelectYesNoLogout, token);
