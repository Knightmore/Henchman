...
public static class MessageHandler
{
    ...
    public static async Task<(CommandType header, string json)?> ReadMessageAsync(PipeStream pipe, CancellationToken token)
    {
        ...
        try
        {
            ...
            await pipe.ReadExactlyAsync(timestampBuffer, 0, 8, token)
            ...
            await pipe.ReadExactlyAsync(headerBuffer, 0, 2, token)
                      .ConfigureAwait(false);
            ;
            ...
            await pipe.ReadExactlyAsync(lengthBuffer, 0, 4, token)
                      .ConfigureAwait(false);
            ;
            ...
            await pipe.ReadExactlyAsync(payloadBuffer, 0, payloadLength, token)
                      .ConfigureAwait(false);
            ;
    ...
    public static async Task WriteMessageAsync(PipeStream pipe, CommandType commandType, string json, CancellationToken token)
    {
        try
        {
            if (token.IsCancellationRequested) VerboseSpecific("MessageWriter", "Cancellation requested before WriteAsync");
            ...
            await pipe.WriteAsync(messageBytes, 0, totalLength, token)
                      .ConfigureAwait(false);
            ...
            await Task.Run(() => pipe.WaitForPipeDrain(), token)
                      .ConfigureAwait(false);
